...
router.post("/:id/like", passport.authenticate("jwt", { session: false }), (req, res) => {
	Profile.findOne({ user: req.user.id }).then(profile => {
		Post.findById(req.params.id)
			.then(post => {
				const filteredPostLikes = post.likes.filter(like => like.user.toString() === req.user.id.toString());
				if (filteredPostLikes.length > 0) {
...
router.delete("/:id/comment/:comment_id", passport.authenticate("jwt", { session: false }), (req, res) => {
	Post.findById(req.params.id)
		.then(post => {
			const filteredPostComments = post.comments.filter(comment => comment._id.toString() === req.params.comment_id);
			if (filteredPostComments.length === 0) {
				return res.status(404).json({ commentdoesnotexist: "Comment doesn't exist" })
			}
